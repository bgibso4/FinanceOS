...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to find potential returns", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
