...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ month: string; categoryId: string }> }) {
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ month: string; categoryId: string }> }) {
  const { month, categoryId } = await params;
