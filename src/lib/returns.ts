...
export async function findPotentialReturns(prisma: PrismaClient, transactionId: string) {
  ...
  const scored = uniqueMatches.map((match) => {
    ...
    const daysDiff =
      Math.abs(match.date.getTime() - transaction.date.getTime()) / (1000 * 60 * 60 * 24);
    ...
    const exactMerchantBonus =
      match.merchantNormalized === transaction.merchantNormalized ? 0.2 : 0;
  ...
  console.log(
    'Final results:',
    results.length,
    results.map((r) => ({ merchant: r.merchant, score: r.score }))
  );
...
export async function unlinkReturn(prisma: PrismaClient, returnTransactionId: string) {
  await prisma.transaction.update({
    where: { id: returnTransactionId },
    data: {
      isOffset: false,
