...
function createImportHash(accountId: string, date: Date, amount: number, merchantNormalized: string): string {
...
export async function importCsv(
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    if (parts.length !== 3) {
      throw new Error(`Row ${rowNum}: Invalid date format "${dateStr}". Expected format: YYYY-MM-DD, MM/DD/YYYY, or MM/DD/YY`);
    }
    ...
    if (isNaN(date.getTime())) {
      throw new Error(`Row ${rowNum}: Invalid date "${dateStr}" (parsed as ${year}-${month}-${day})`);
    }
    ...
    if (date < minDate || date > maxDate) {
      throw new Error(`Row ${rowNum}: Date "${dateStr}" is outside valid range (2020 to 1 year ahead). Parsed as ${date.toISOString().split('T')[0]}. Check if year format is correct (use 4-digit years or MM/DD/YY format).`);
    }
    ...
    if (await isDuplicate(prisma, accountId, date, amount, merchantNormalized, externalId, importHash)) {
      duplicates.push({
        merchant,
        amount: finalAmount,
...
async function detectTransfers(prisma: PrismaClient, accountId: string, newTransactionIds: Set<string>) {
    ...
    for (const tx of group) {
      ...
      if (match) {
        ...
        if (newTransactionIds.has(tx.id) || newTransactionIds.has(match.id)) {
          ...
          console.log(`  ✓ Same-account transfer: ${tx.merchant} $${tx.amount} + ${match.merchant} $${match.amount}`);
        }
      }
    }
  }

  console.log(`  Found ${sameAccountMatches} same-account transfer pairs involving new transactions`);

  // Second: detect cross-account transfers (e.g., credit card payments)
  const { crossAccountMatches, crossAccountTransfers } = await detectCrossAccountTransfers(prisma, newTransactionIds);
...
async function detectCrossAccountTransfers(prisma: PrismaClient, newTransactionIds: Set<string>) {
  ...
  const candidates = allRecent.filter(tx => {
    const hasOpposite = allRecent.some(other => 
      other.id !== tx.id &&
  ...
  if (candidates.length > 0) {
    ...
    candidates.forEach(tx => {
      console.log(`    - ${tx.account.name}: ${tx.merchant} $${tx.amount} on ${tx.date.toISOString().split('T')[0]} (isTransfer: ${tx.isTransfer})`);
    });
  }
  ...
  for (const tx of allRecent) {
    ...
    const match = allRecent.find((other) => {
      ...
      const transferKeywords = ['payment', 'transfer', 'xfer', 'autopay', 'bill pay', 'credit card'];
    ...
    if (match) {
      ...
      if (newTransactionIds.has(tx.id) || newTransactionIds.has(match.id)) {
        ...
        console.log(`  ✓ Cross-account transfer: ${tx.account.name} $${tx.amount} ↔ ${match.account.name} $${match.amount}`);
      }
    }
  }
  
  console.log(`  Found ${crossAccountMatches} cross-account transfer pairs involving new transactions`);
